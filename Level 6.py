...
def do_turn(game):
	...
	for pirate in game.my_pirates_without_treasures():
		if attackbest(game, pirate)==True:
			game.debug("Attacked")
		elif kamikaza(game,pirate,moves)==True:
			game.debug("We did Kamikaza")
		else:
			...
			for enemy in game.enemy_pirates():
				if game.in_range(pirate, enemy) and enemy.has_treasure:
					if enemy.treasure_value>best_attack.treasure_value:
						best_attack=enemy
			game.debug("make kamikaza")
			go_to_enemy(game,pirate,best_attack,moves)
			return True
		else:
			return False
	else:
...
def go_to_enemy(game,pirate,enemy,moves):
	...
	if distance >= moves:
		...
		game.debug("run after enemy to kamikaza him")
		found = False
		for i in destinations:
			if not found:
				if (game.get_pirate_on(i) not in game.my_pirates()) and (i not in locations):		
					if pirate.turns_to_sober > 0:
						...
						game.set_sail(pirate, i)
						moves = moves - moves#make moves equal to 0
						locations.append(i)
						game.debug(moves)
						found = True
	else:
